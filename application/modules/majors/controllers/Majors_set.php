...
// Nip
  ...
  public function add_glob(){
    ...
    $this->session->set_flashdata('success',' Tambah Rayon Berhasil');
...
// Add User_customer and Update
  public function add($id = NULL) {
    ...
    $this->form_validation->set_rules('majors_name', 'Nama Rayon', 'trim|required|xss_clean');
    $this->form_validation->set_rules('majors_short_name', 'Singkatan Rayon', 'trim|required|xss_clean');
    ...
    if ($_POST AND $this->form_validation->run() == TRUE) {
      ...
      $data['title'] = $data['operation'] . ' Rayon';
...
// Delete to database
  public function delete($id = NULL) {
    ...
    if ($_POST) {
      ...
      if (count($siswa) > 0) {
        $this->session->set_flashdata('failed', 'Data Rayon tidak dapat dihapus');
        redirect('manage/majors');
      }
...
// activity log
      ...
      $this->session->set_flashdata('success', 'Hapus Rayon berhasil');
      redirect('manage/majors');
    } elseif (!$_POST) {
