...
class Payout_set extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->load->model(array('payment/Payment_model', 'student/Student_model', 'period/Period_model', 'pos/Pos_model', 'bulan/Bulan_model', 'bulan/Bulan_pay_model', 'bebas/Bebas_model', 'bebas/Bebas_pay_model', 'setting/Setting_model', 'letter/Letter_model', 'logs/Logs_model', 'ltrx/Log_trx_model', 'month/Month_model'));
...
// Siswa
    ...
    // echo "<pre>".print_r($data['log'], true)."<pre>";
...
// View data detail
  ...
  public function payout_bulan($id = NULL, $student_id = NULL, $bulan_id = NULL, $pay_id = NULL) {
    // if ($id == NULL AND $student_id == NULL AND $bulan_id == NULL OR $bulan_id == NULL) {
    //   redirect('manage/payout');
    // }
    if ($_POST == TRUE) {
      ...
      $student = $this->Bulan_model->get(array('id'=>$this->input->post('bulan_id')));
      ...
      if ($this->input->post('bulan_id')) {
        $param['bulan_id'] = $this->input->post('bulan_id');
      } 
      $param['bulan_pay_number'] = $nofull;
      $param['bulan_pay_bill'] = $this->input->post('bulan_pay_bill');
      $param['increase_budget'] = $this->input->post('bulan_pay_bill');
      $param['bulan_pay_desc'] = $this->input->post('bulan_pay_desc');
      $param['user_user_id'] = $this->session->userdata('uid');
      $param['bulan_pay_input_date'] = date('Y-m-d H:i:s');
      $param['bulan_pay_last_update'] = date('Y-m-d H:i:s');

      

      $data['bill'] = $this->Bulan_pay_model->get(array('bulan_id'=>$this->input->post('bulan_id')));
      $data['bulan'] = $this->Bulan_model->get(array('payment_id' => $this->input->post('payment_payment_id'), 'student_nis' => $this->input->post('student_nis')));
      ...
      if ($this->input->post('bulan_pay_bill') > $sisa OR $this->input->post('bulan_pay_bill') == 0) {
        ...
        $idd = $this->Bulan_pay_model->add($param);

        $this->Bulan_model->add(array('increase_budget' => $this->input->post('bulan_pay_bill'), 'bulan_id' =>  $this->input->post('bulan_id'), 'bulan_last_update'=>date('Y-m-d H:i:s'))); 
        
        $log = array(
          'bulan_pay_bulan_pay_id' => $idd,
      ...
      if ($activated['setting_value'] == 'Y') {
        ...
        $message = "Pembayaran ".$student['pos_name'].' - T.A '.$student['period_start'].'/'.$student['period_end'].'-'.$this->input->post('bulan_pay_desc').' a/n '.$student['student_full_name'].' Berhasil';
  ...
  function delete_pay_bebas($payment_id = NULL, $student_id =NULL, $bebas_id = NULL, $id =NULL) {
  ...
  function delete_pay_bulan($payment_id = NULL, $student_id =NULL, $bulan_id = NULL, $id =NULL) {

    $total_pay = $this->Bulan_pay_model->get(array('id'=>$id));

    $this->Bulan_model->add(
      array(
        'decrease_budget'=> $total_pay['bulan_pay_bill'],
        'bulan_id'=>$bulan_id
      )
    );
    ...
    $this->Bulan_pay_model->delete($id);

    if ($this->input->is_ajax_request()) {
      echo $status;
    } else {
      $this->session->set_flashdata('success', 'Delete Berhasil');
      redirect('manage/payout/payout_bulan/' . $payment_id.'/'.$student_id.'/'.$bulan_id);
    }
