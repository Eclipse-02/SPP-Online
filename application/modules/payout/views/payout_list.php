<div class="content-wrapper">
	...
	<section class="content">
					...
					<div class="box-body">
						...
						<div class="form-group">						
							...
							<div class="col-sm-2" style="margin: 5px 0">
								<select class="form-control" name="n" id="period">
							...
							<div class="col-sm-3">
								<div class="input-group">
									<select class="form-control" name="r" id="nis_siswa" required>
										<option value="">Masukan NIS Siswa</option>
										<?php foreach ($siswa as $row): ?>
											<option <?php echo (isset($f['r']) AND $f['r'] == $row['student_nis']) ? 'selected' : '' ?> value="<?php echo $row['student_nis'] ?>"><?php echo $row['student_nis'] ?></option>
										<?php endforeach; ?>
									</select>
														...
														<tr>
															...
															<?php foreach ($bulan as $row) : ?>
																<?php if ($r['payment_payment_id'] == $row['payment_payment_id']) {?>
																		<!-- <td class="<?php echo ($row['bulan_status'] ==1) ? 'success' : 'danger' ?>">
																			<a href="<?php echo ($row['bulan_status'] ==0) ? site_url('manage/payout/pay/' . $row['payment_payment_id'].'/'.$row['student_student_id'].'/'.$row['bulan_id']) : site_url('manage/payout/not_pay/' . $row['payment_payment_id'].'/'.$row['student_student_id'].'/'.$row['bulan_id'])?>" onclick="return confirm('<?php echo ($row['bulan_status']==0) ? 'Anda Akan Melakukan Pembayaran bulan '.$row['month_name'].'?' : 'Anda Akan Menghapus Pembayaran bulan'.$row['month_name'].'?' ?>')">
																				<?php echo ($row['bulan_status']==1) ? '('.pretty_date($row['bulan_date_pay'],'d/m/y',false).')': number_format($row['bulan_bill'], 0, ',', '.') ?>
																			</a>
																		</td> -->
		...
		<link rel="stylesheet" href="<?php echo media_url() ?>/css/select2.css">
