						...
						if (!empty($bill)) {
							...
							foreach ($bill as $row): 
								...
								<tr>
									...
									<td>
										<a class="btn btn-danger btn-xs" onclick="return confirm('Anda Akan Menghapus Pembayaran <?php echo $row['bebas_pay_desc'] ?> ?')" title="Hapus" href="<?php echo site_url('manage/payout/delete_pay_bebas/'.$row['payment_payment_id'].'/'.$row['student_student_id'].'/'.$row['bebas_bebas_id'].'/'.$row['bebas_pay_id']) ?>""><span class="glyphicon glyphicon-trash"></span></a>
									</td>
								</tr>
